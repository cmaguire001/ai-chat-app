...
export default function Home() {
  ...
  return (
    <div
      className={`theme-${theme} app-bg min-h-screen flex flex-col items-center justify-center p-4 relative ${
        isUnderwater ? 'underwater-bg' : ''
      }`}
      style={{ minHeight: '100dvh' }}
    >
      {/* Animated bubbles (underwater only) */}
      {isUnderwater && <Bubbles />}

      {/* Outer container */}
      <div
        className={`w-full max-w-2xl flex flex-col relative z-10 ${
          isUnderwater ? 'wave-container' : ''
        }`}
        style={{ height: 'min(720px, 92dvh)' }}
      >
        {/* Header */}
        <div
          className={`app-surface app-border border-b px-5 py-4 rounded-t-2xl flex flex-col gap-3 ${
            isUnderwater ? 'glass' : ''
          }`}
          style={{
            borderBottom: '1px solid var(--border)',
            background: isUnderwater ? 'var(--surface)' : undefined,
          }}
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-display font-700 text-base leading-tight" style={{ color: 'var(--text)' }}>
                  AI Chat
                </h1>
        ...
        {/* Chat area */}
        <div
          className={`flex-1 overflow-hidden ${isUnderwater ? 'glass' : 'app-surface'}`}
        ...
        {/* Input area */}
        <div
          className={`app-surface app-border border-t px-4 py-3 rounded-b-2xl ${
            isUnderwater ? 'glass' : ''
          }`}
          style={{
        ...
        >
          <div className="flex gap-2 items-end">
            <textarea
              className="app-input flex-1 rounded-xl px-4 py-2.5 text-sm resize-none border focus:ring-2"
              style={{
                borderWidth: '1.5px',
                minHeight: '44px',
                maxHeight: '120px',
